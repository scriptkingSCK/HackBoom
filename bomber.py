...
try:
    ...
    print('[!] Error: กรุณาติดตั้งโมดูลก่อนใช้งาน')
    print('Type \'pip install -r requirements.txt\' ติดตั้งโมดูลเพื่อใช้งาน')
    exit()
...
def start(target, counter, delay, ch, cc):
    ...
    while success < (int(bombs)):
        ...
        try:
            ...
            if cc == "91":
                print('กรุณาอัพเดทก่อน')
                input('Press Enter To Exit...')
                exit()
            else:
                if success > 0:
                    print(
                        '\n\n\tเราต้องขออภัยที่จะ จำกัด การวางระเบิดสำหรับประเทศของคุณ ......')
                    print(
                        '\nเรากำลังทำงานหนักเกินไปที่จะเพิ่มขีด จำกัด...')
                    input(
                        '\nสิ่งนี้จะช่วยให้เราสามารถให้การสนับสนุนประเทศของคุณอย่างรวดเร็ว...\n\nPress Enter To Exit...')
        ...
        print("==================================================================")
        print("                ระบบกำลังทำงานความเร็วจะขึ้นกับข้อมูล!!               ")
        print("     กรุณาเปิดอินเทอร์เน็ตไว้เพื่อทำการยิง!!    ")
        print("==================================================================")
        print("             จำนวนเป้าหมาย          : +" + str(cc) + " ", target)
        print("             จำนวนคำขอที่ส่ง : ", requested)
...
try:
    ...
    print("คุณไม่มีอินเทอร์เน็ต!!!")
    print("\tกรุณาเชื่อมต่ออินเทอร์เน็ตเพื่อทำงานต่อ..\n")
    input('Exiting....\n Press Enter To Continue....')
    exit()
print('\tเช็คอัพเดท...')
ver = urllib.request.urlopen(
    "https://raw.githubusercontent.com/scriptkingSCK/HackBoom/master/.version").read().decode('utf-8')
verl = ''
try:
...
try:
    noti = urllib.request.urlopen(
        "https://raw.githubusercontent.com/scriptkingSCK/HackBoom/master/.notify").read().decode('utf-8')
...
try:
    ...
    type = 0
if type == 1:
    nm = int(input("ใส่จำนวน(จำกัด 15): "))
    if nm > 15:
        print("\t\tYou Have Entered " + str(nm) +
              ".\n\tNormalizing Value To 15")
        nm = 15
    dl = float(input("ใส่เวลา [Recommended 10 sec ] : "))
elif type == 0:
    if cc == "91":
        nm = int(input("ใส่เบอร์ยิงข้อความ(0 For Unlimited): "))
        dl = float(
            input("ดีเลย์ (in seconds) [Recommended 2 sec ] : "))
    else:
        nm = int(input("จำนวนจะส่ง: "))
        dl = float(
            input("เวลายิง (in seconds) [Recommended 10 sec ] : "))
maxlim = 0
if cc == "91":
    maxlim = 500
else:
